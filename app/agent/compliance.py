...
from .types import SafetyInfo, SafetyLevel, AnswerPayload
...
CREDENTIAL_PATTERNS = re.compile(
    r"(password|api[_-]?key|secret|token|auth|login|credential|private key)", 
    re.I
)
...
class RegexSafetyChecker(SafetyChecker):
    ...
    def _apply_safety_modifications(
        ...
        if result.confidence_penalty > 0:
            ...
            logger.debug(f"Confidence reduced from {payload.confidence + result.confidence_penalty} to {new_confidence}")
