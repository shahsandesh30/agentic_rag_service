...
from .interfaces import DatabaseManager, DocumentData, ChunkData, ProcessingStats
...
def fetch_chunk_texts(conn: sqlite3.Connection, chunk_ids: List[str], batch_size: int = DEFAULT_BATCH_SIZE) -> Dict[str, Dict]:
    """
    ...
    try:
        ...
        for i in range(0, len(chunk_ids), batch_size):
            ...
            for row in conn.execute(query, batch):
                ...
                doc = conn.execute("SELECT path, source FROM documents WHERE id=?", (row["doc_id"],)).fetchone()
...
def fetch_full_chunks(conn: sqlite3.Connection, chunk_ids: List[str], batch_size: int = DEFAULT_BATCH_SIZE) -> Dict[str, str]:
    """
...
def chunk_ids_for_faiss_ids(conn: sqlite3.Connection, faiss_ids: List[int], batch_size: int = DEFAULT_BATCH_SIZE) -> Dict[int, str]:
    """
