...
from .interfaces import FileIterator, DocumentProcessor, FileMetadata
...
def _read_text_file(path: str) -> Tuple[str, int]:
    ...
    try:
        with open(path, "r", encoding=DEFAULT_ENCODING, errors=DEFAULT_ERROR_HANDLING) as f:
            content = f.read()
        ...
        try:
            with open(path, "r", encoding="latin-1", errors=DEFAULT_ERROR_HANDLING) as f:
                content = f.read()
