...
from .interfaces import ProcessingStats, DocumentData, ChunkData
...
def _chunk_id(doc_id: str, start: int, end: int) -> str:
    ...
    """
    try:
        chunk_id = hashlib.sha256(f"{doc_id}:{start}:{end}".encode("utf-8")).hexdigest()[:24]
...
def ingest_path(
    ...
    logger.info(f"Parameters: max_chars={max_chars}, overlap={overlap}, heading_aware={heading_aware}")
...
def _process_document_metadata(file_path: str, source: str) -> DocumentData:
    ...
    try:
        ...
        data_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", DEFAULT_DATA_ROOT))
...
def _get_or_create_document_id(conn, doc_metadata: DocumentData) -> str:
    ...
    try:
        ...
        row = conn.execute("SELECT id FROM documents WHERE sha256 = ?", (doc_metadata["sha256"],)).fetchone()
        if row:
...
def _prepare_chunk_rows(doc_id: str, chunks: List[ChunkData], doc_metadata: DocumentData) -> List[Tuple]:
    """
